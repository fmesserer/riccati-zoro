...
class SolverEllipsoidOcpOptimalStatefb(SolverNominalOcp):
    ...
    def _build_solver(self) -> None:
        ...
        # uncertainty decision variables
        ...
        kfb_vec = ca.SX.sym("kvec", self.ocp.nu * self.ocp.nx)  # feedback gain as vector
        ...
        # direct cost on uncertainty
        Q_unc_stage = self.ocp.cost_uncertainty_Q_stage
        R_unc = self.ocp.cost_uncertainty_R
        S_unc = self.ocp.cost_uncertainty_S
        Q_unc_terminal = self.ocp.cost_uncertainty_Q_terminal
        C_unc_stage = ca.blockcat([[Q_unc_stage, S_unc.T], [S_unc, R_unc]])

        cost_unc_stage = ca.trace(C_unc_stage @ ca.blockcat([[p, p @ kfb.T], [kfb @ p, kfb @ p @ kfb.T]]))
        cost_unc_stage_func = ca.Function("cost_unc_stage", [pvec, kfb_vec], [cost_unc_stage])
        cost_unc_terminal = ca.trace(Q_unc_terminal @ p)
        cost_unc_terminal_func = ca.Function("cost_unc_terminal", [pvec], [cost_unc_terminal])
        ...
        dyn_ellipsoid = ca.Function("dyn_ellipsoid", [self.ocp.x, self.ocp.u, self.ocp.w, pvec, kfb_vec, self.ocp.par], [p_next_vec])
        ...
        constr_jac += ca.jacobian(self.ocp.constr_stage_expr, self.ocp.u) @ kfb
        ...
        backoff_sq_func = ca.Function("backoff_sq", [self.ocp.x, self.ocp.u, pvec, kfb_vec, self.ocp.par], [backoff_sq_expr])
        ...
        Kvec = ca.SX.sym("Kvec", self.ocp.nu * self.ocp.nx, self.ocp.N)  # feedback gain as vector for each stage
        ...
        # vector of all decision variables ordered as [x0, pvec0, beta0, u0, K0, x1, u1, K1, ..., xN, pvecN, betaN]
        decvar = ca.veccat(X[:,0], Pvec[:,0], ca.vertcat(Beta, U, Kvec, X[:,1:], Pvec[:,1:]), beta_N)
        # to extract trajectories in nice shape from decvar
        self._extract_traj = ca.Function("extract_traj", [decvar], [X, U, Pvec, Kvec, Beta, beta_N])
        self._traj_to_vec = ca.Function("traj_to_vec", [X, U, Pvec, Kvec, Beta, beta_N], [decvar])
        ...
        lb_Kvec = -np.inf * np.ones(Kvec.shape)
        ub_Kvec = np.inf * np.ones(Kvec.shape)
        ...
        decvar_lb = self._traj_to_vec(lb_X, lb_U, lb_Pvec, lb_Kvec, lb_Beta, lb_beta_N).full().flatten()
        decvar_ub = self._traj_to_vec(ub_X, ub_U, ub_Pvec, ub_Kvec, ub_Beta, ub_beta_N).full().flatten()
        self._lbx = decvar_lb
        self._ubx = decvar_ub
        ...
        for k in range(self.ocp.N):
            ...
            obj += cost_unc_stage_func(Pvec[:,k], Kvec[:,k])
            obj += 1e-4 * ca.sum1(Beta[:,k])  # small penalty to push actively into constraint
            obj += 1e-4 * ca.sum1(Kvec[:,k]**2)  # numerical regularization of feedback gain


            x_next = dyn_discr(X[:,k], U[:,k], 0, Par[:,k])
            p_next_vec = dyn_ellipsoid(X[:,k], U[:,k], 0, Pvec[:,k], Kvec[:,k], Par[:,k])
            ...
            constr.append(Beta[:,k] - backoff_sq_func(X[:,k], U[:,k], Pvec[:,k], Kvec[:,k], Par[:,k]))
        ...
        obj += cost_unc_terminal_func(Pvec[:,self.ocp.N])
    ...
    def solve(self, x0: np.ndarray, P0: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray, List[np.ndarray], List[np.ndarray]]:
        '''
        ...
        print("---------------------------------------")
        print("Feedback-optimization OCP with IPOPT")
        ...
        Xopt, Uopt, P_opt, K_opt, Beta_opt, beta_N_opt = self._extract_traj(self._sol["x"])
        return Xopt.full(), Uopt.full(), self.Pvec_traj_to_P_list(P_opt.full()), self.Kvec_traj_to_K_list(K_opt.full())

    def set_initial_guess(self, X: np.ndarray, U: np.ndarray, P: np.ndarray, K: np.ndarray = 0, Beta: np.ndarray = 1e-4, beta_N: np.ndarray = 1e-4) -> None:
        '''
        ...
        K: control feedback gain trajectory of shape (nu*nx, N)
        ...
        '''
        self._init_guess = self._traj_to_vec(X, U, P, K, Beta, beta_N).full().flatten()
